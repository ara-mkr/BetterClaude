...
var require_dist2 = __commonJS({
  "node_modules/@codemirror/state/dist/index.cjs"(exports2) {
    ...
    exports2.EditorState = EditorState2;
    exports2.Facet = Facet;
    exports2.Line = Line;
    exports2.Prec = Prec;
    exports2.Range = Range;
    exports2.RangeSet = RangeSet;
    exports2.RangeSetBuilder = RangeSetBuilder;
...
var require_style_mod = __commonJS({
  "node_modules/style-mod/dist/style-mod.cjs"(exports2) {
    ...
    StyleModule.mount = function mount2(root, modules, options) {
    ...
    StyleSet.prototype.mount = function mount2(modules, root) {
...
var require_dist4 = __commonJS({
  "node_modules/@codemirror/view/dist/index.cjs"(exports2) {
    ...
    var EditorView2 = class _EditorView {
      /**
    ...
    EditorView2.cursorScrollMargin = state.Facet.define({
      combine: (inputs) => {
        let x = 5, y = 5;
        for (let i of inputs) {
          if (typeof i == "number")
    ...
    EditorView2.cspNonce = state.Facet.define({ combine: (values) => values.length ? values[0] : "" });
    ...
    var keymap2 = state.Facet.define({ enables: handleKeyEvents });
    var Keymaps = /* @__PURE__ */ new WeakMap();
    function getKeymap(state2) {
      let bindings = state2.facet(keymap2);
    ...
    var hideNativeSelection = state.Prec.highest(EditorView2.theme({
      ".cm-line": {
    ...
    function placeholder(content) {
      ...
      return typeof content == "string" ? [
        plugin2,
        EditorView2.contentAttributes.of({ "aria-placeholder": content })
      ] : plugin2;
    }
    var MaxOff = 2e3;
    ...
    exports2.highlightActiveLine = highlightActiveLine2;
    ...
    exports2.lineNumbers = lineNumbers2;
  ...
  "node_modules/@lezer/common/dist/index.cjs"(exports2) {
      ...
      findMounts(pos, parser) {
        ...
        if (this.inner) {
          ...
          for (let pos2 = this.inner.cursor.node; pos2; pos2 = pos2.parent) {
            let mount2 = (_a = pos2.tree) === null || _a === void 0 ? void 0 : _a.prop(NodeProp.mounted);
            if (mount2 && mount2.parser == parser) {
    ...
    function enterFragments(mounts, ranges) {
      ...
      for (let { pos, mount: mount2, frag } of mounts) {
        ...
        if (mount2.overlay) {
          let overlay = mount2.overlay.map((r) => new Range(r.from + pos, r.to + pos));
          let changes = findCoverChanges(ranges, overlay, from, to);
          for (let i = 0, pos2 = from; ; i++) {
            let last = i == changes.length, end = last ? to : changes[i].from;
            if (end > pos2)
              result.push(new TreeFragment(pos2, end, mount2.tree, -startPos, frag.from >= pos2 || frag.openStart, frag.to <= end || frag.openEnd));
            if (last)
              break;
            pos2 = changes[i].to;
          }
        } else {
          result.push(new TreeFragment(from, to, mount2.tree, -startPos, frag.from >= startPos || frag.openStart, frag.to <= endPos || frag.openEnd));
        }
      }
      return result;
    }
...
var require_dist6 = __commonJS({
  "node_modules/@lezer/highlight/dist/index.cjs"(exports2) {
    "use strict";
    var common = require_dist5();
    var nextTagID = 0;
    var Tag = class _Tag {
      /**
      @internal
    ...
    exports2.Tag = Tag;
    exports2.classHighlighter = classHighlighter;
    exports2.getStyleTags = getStyleTags;
    exports2.highlightCode = highlightCode;
    exports2.highlightTree = highlightTree;
    exports2.styleTags = styleTags;
    exports2.tagHighlighter = tagHighlighter;
    exports2.tags = tags;
  }
});
...
var require_dist7 = __commonJS({
  "node_modules/@codemirror/language/dist/index.cjs"(exports2) {
    "use strict";
    var common = require_dist5();
    var state = require_dist2();
    var view = require_dist4();
    var highlight = require_dist6();
    var styleMod = require_style_mod();
    var _a;
      ...
      */
      findRegions(state2) {
        ...
        let explore = (tree, from) => {
          ...
          let mount2 = tree.prop(common.NodeProp.mounted);
          if (mount2) {
      ...
      update(folded, tr) {
        ...
        if (rangesToFold.length) {
          ...
          let decorations = rangesToFold.map((value) => {
            let widget = !preparePlaceholder ? foldWidget : view.Decoration.replace({ widget: new PreparedFoldWidget(preparePlaceholder(tr.state, value)) });
    ...
    function bracketDeco(state2) {
      let decorations = [];
      let config = state2.facet(bracketMatchingConfig);
      for (let range of state2.selection.ranges) {
        if (!range.empty)
          continue;
        let match = matchBrackets(state2, range.head, -1, config) || range.head > 0 && matchBrackets(state2, range.head - 1, 1, config) || config.afterCursor && (matchBrackets(state2, range.head, 1, config) || range.head < state2.doc.length && matchBrackets(state2, range.head + 1, -1, config));
        if (match)
          decorations = decorations.concat(config.renderMatch(match, state2));
      }
      return view.Decoration.set(decorations, true);
    }
    var bracketMatcher = view.ViewPlugin.fromClass(class {
      constructor(view2) {
        this.paused = false;
    ...
    function matchPlainBrackets(state2, pos, dir, tree, tokenType, maxScanDistance, brackets) {
      if (dir < 0 ? !pos : pos == state2.doc.length)
        return null;
    ...
    var StreamLanguage = class _StreamLanguage extends Language {
      ...
      */
      getIndent(cx) {
        ...
        let start = findState(this, cx.node.tree, cx.node.from, cx.node.from, from !== null && from !== void 0 ? from : cx.pos), statePos, state2;
        if (start) {
          state2 = start.state;
    ...
    exports2.DocInput = DocInput;
    exports2.HighlightStyle = HighlightStyle;
    exports2.IndentContext = IndentContext;
    exports2.LRLanguage = LRLanguage;
    exports2.Language = Language;
    exports2.LanguageDescription = LanguageDescription;
    exports2.LanguageSupport = LanguageSupport;
    exports2.ParseContext = ParseContext;
    exports2.StreamLanguage = StreamLanguage;
    exports2.StringStream = StringStream;
    exports2.TreeIndentContext = TreeIndentContext;
    exports2.bidiIsolates = bidiIsolates;
    exports2.bracketMatching = bracketMatching;
    exports2.bracketMatchingHandle = bracketMatchingHandle;
    exports2.codeFolding = codeFolding;
    exports2.continuedIndent = continuedIndent;
    exports2.defaultHighlightStyle = defaultHighlightStyle;
    exports2.defineLanguageFacet = defineLanguageFacet;
    exports2.delimitedIndent = delimitedIndent;
    exports2.ensureSyntaxTree = ensureSyntaxTree;
    exports2.flatIndent = flatIndent;
    exports2.foldAll = foldAll;
    exports2.foldCode = foldCode;
    exports2.foldEffect = foldEffect;
    exports2.foldGutter = foldGutter;
    exports2.foldInside = foldInside;
    exports2.foldKeymap = foldKeymap;
    exports2.foldNodeProp = foldNodeProp;
    exports2.foldService = foldService;
    exports2.foldState = foldState;
    exports2.foldable = foldable;
    exports2.foldedRanges = foldedRanges;
    exports2.forceParsing = forceParsing;
    exports2.getIndentUnit = getIndentUnit;
    exports2.getIndentation = getIndentation;
    exports2.highlightingFor = highlightingFor;
    exports2.indentNodeProp = indentNodeProp;
    exports2.indentOnInput = indentOnInput;
    exports2.indentRange = indentRange;
    exports2.indentService = indentService;
    exports2.indentString = indentString;
    exports2.indentUnit = indentUnit;
    exports2.language = language;
    exports2.languageDataProp = languageDataProp;
    exports2.matchBrackets = matchBrackets;
    exports2.sublanguageProp = sublanguageProp;
    exports2.syntaxHighlighting = syntaxHighlighting;
    exports2.syntaxParserRunning = syntaxParserRunning;
    exports2.syntaxTree = syntaxTree;
    exports2.syntaxTreeAvailable = syntaxTreeAvailable;
    exports2.toggleFold = toggleFold;
    exports2.unfoldAll = unfoldAll;
    exports2.unfoldCode = unfoldCode;
    exports2.unfoldEffect = unfoldEffect;
  }
});

// node_modules/@codemirror/commands/dist/index.cjs
var require_dist8 = __commonJS({
  "node_modules/@codemirror/commands/dist/index.cjs"(exports2) {
    "use strict";
    var state = require_dist2();
    var view = require_dist4();
    ...
    function getConfig(state2, pos) {
      let data = state2.languageDataAt("commentTokens", pos, 1);
    ...
    function changeLineComment(option, state2, ranges = state2.selection.ranges) {
      ...
      ranges: for (let { from, to } of ranges) {
        ...
        for (let pos = from; pos <= to; ) {
          let line = state2.doc.lineAt(pos);
          if (token == void 0) {
            token = getConfig(state2, line.from).line;
            if (!token)
              continue ranges;
          }
    ...
    var HistoryState = class _HistoryState {
      ...
      pop(side, state2, onlySelection) {
        ...
        if (branch.length == 0)
          return null;
        let event = branch[branch.length - 1], selection = event.selectionsAfter[0] || (event.startSelection ? event.startSelection.map(event.changes.invertedDesc, 1) : state2.selection);
    ...
    function toGroupStart(view2, pos, start) {
    ...
    var cursorGroupForwardWin = (view2) => {
      return moveSel(view2, (range) => range.empty ? view2.moveByChar(range, true, (start) => toGroupStart(view2, range.head, start)) : rangeEnd(range, true));
    };
    ...
    function extendSel(target, forward, how) {
      let selection = updateSel(target.state.selection, (range) => {
        ...
        let head = how(range);
        return state.EditorSelection.range(range.anchor, head.head, head.goalColumn, head.bidiLevel || void 0, head.assoc);
      });
    ...
    var selectGroupForwardWin = (view2) => {
      return extendSel(view2, true, (range) => view2.moveByChar(range, true, (start) => toGroupStart(view2, range.head, start)));
    };
    function selectBySubword(view2, forward) {
      return extendSel(view2, forward, (range) => moveBySubword(view2, range, forward));
    }
    ...
    var selectLineStart = (view2) => extendSel(view2, false, (range) => state.EditorSelection.cursor(view2.lineBlockAt(range.head).from));
    var selectLineEnd = (view2) => extendSel(view2, true, (range) => state.EditorSelection.cursor(view2.lineBlockAt(range.head).to));
    ...
    function addCursorVertically(view2, forward) {
      let { state: state$1 } = view2, sel = state$1.selection, ranges = state$1.selection.ranges.slice();
      for (let range of state$1.selection.ranges) {
      ...
      if (ranges.length == sel.ranges.length)
        return false;
      view2.dispatch(setSel(state$1, state.EditorSelection.create(ranges, ranges.length - 1)));
      return true;
    }
    ...
    var deleteGroupForwardWin = (view2) => deleteBy(view2, (range) => view2.moveByChar(range, true, (start) => toGroupStart(view2, range.head, start)).head);
    ...
    exports2.blockUncomment = blockUncomment;
    exports2.copyLineDown = copyLineDown;
    exports2.copyLineUp = copyLineUp;
    exports2.cursorCharBackward = cursorCharBackward;
    exports2.cursorCharBackwardLogical = cursorCharBackwardLogical;
    exports2.cursorCharForward = cursorCharForward;
    exports2.cursorCharForwardLogical = cursorCharForwardLogical;
    exports2.cursorCharLeft = cursorCharLeft;
    exports2.cursorCharRight = cursorCharRight;
    exports2.cursorDocEnd = cursorDocEnd;
    exports2.cursorDocStart = cursorDocStart;
    exports2.cursorGroupBackward = cursorGroupBackward;
    exports2.cursorGroupForward = cursorGroupForward;
    exports2.cursorGroupForwardWin = cursorGroupForwardWin;
    exports2.cursorGroupLeft = cursorGroupLeft;
    exports2.cursorGroupRight = cursorGroupRight;
    exports2.cursorLineBoundaryBackward = cursorLineBoundaryBackward;
    exports2.cursorLineBoundaryForward = cursorLineBoundaryForward;
    exports2.cursorLineBoundaryLeft = cursorLineBoundaryLeft;
    exports2.cursorLineBoundaryRight = cursorLineBoundaryRight;
    exports2.cursorLineDown = cursorLineDown;
    exports2.cursorLineEnd = cursorLineEnd;
    exports2.cursorLineStart = cursorLineStart;
    exports2.cursorLineUp = cursorLineUp;
    exports2.cursorMatchingBracket = cursorMatchingBracket;
    exports2.cursorPageDown = cursorPageDown;
    exports2.cursorPageUp = cursorPageUp;
    exports2.cursorSubwordBackward = cursorSubwordBackward;
    exports2.cursorSubwordForward = cursorSubwordForward;
    exports2.cursorSyntaxLeft = cursorSyntaxLeft;
    exports2.cursorSyntaxRight = cursorSyntaxRight;
    exports2.defaultKeymap = defaultKeymap2;
    exports2.deleteCharBackward = deleteCharBackward;
    exports2.deleteCharBackwardStrict = deleteCharBackwardStrict;
    exports2.deleteCharForward = deleteCharForward;
    exports2.deleteGroupBackward = deleteGroupBackward;
    exports2.deleteGroupForward = deleteGroupForward;
    exports2.deleteGroupForwardWin = deleteGroupForwardWin;
    exports2.deleteLine = deleteLine;
    exports2.deleteLineBoundaryBackward = deleteLineBoundaryBackward;
    exports2.deleteLineBoundaryForward = deleteLineBoundaryForward;
    exports2.deleteToLineEnd = deleteToLineEnd;
    exports2.deleteToLineStart = deleteToLineStart;
    exports2.deleteTrailingWhitespace = deleteTrailingWhitespace;
    exports2.emacsStyleKeymap = emacsStyleKeymap;
    exports2.history = history2;
    exports2.historyField = historyField;
    exports2.historyKeymap = historyKeymap2;
    ...
    exports2.insertNewlineKeepIndent = insertNewlineKeepIndent;
    exports2.insertTab = insertTab;
    exports2.invertedEffects = invertedEffects;
    exports2.isolateHistory = isolateHistory;
    exports2.lineComment = lineComment;
    exports2.lineUncomment = lineUncomment;
    exports2.moveLineDown = moveLineDown;
    exports2.moveLineUp = moveLineUp;
    exports2.redo = redo;
    exports2.redoDepth = redoDepth;
    exports2.redoSelection = redoSelection;
    exports2.selectAll = selectAll;
    exports2.selectCharBackward = selectCharBackward;
    exports2.selectCharBackwardLogical = selectCharBackwardLogical;
    exports2.selectCharForward = selectCharForward;
    exports2.selectCharForwardLogical = selectCharForwardLogical;
    exports2.selectCharLeft = selectCharLeft;
    exports2.selectCharRight = selectCharRight;
    exports2.selectDocEnd = selectDocEnd;
    exports2.selectDocStart = selectDocStart;
    exports2.selectGroupBackward = selectGroupBackward;
    exports2.selectGroupForward = selectGroupForward;
    exports2.selectGroupForwardWin = selectGroupForwardWin;
    exports2.selectGroupLeft = selectGroupLeft;
    exports2.selectGroupRight = selectGroupRight;
    exports2.selectLine = selectLine;
    exports2.selectLineBoundaryBackward = selectLineBoundaryBackward;
    exports2.selectLineBoundaryForward = selectLineBoundaryForward;
    exports2.selectLineBoundaryLeft = selectLineBoundaryLeft;
    exports2.selectLineBoundaryRight = selectLineBoundaryRight;
    exports2.selectLineDown = selectLineDown;
    exports2.selectLineEnd = selectLineEnd;
    exports2.selectLineStart = selectLineStart;
    exports2.selectLineUp = selectLineUp;
    exports2.selectMatchingBracket = selectMatchingBracket;
    exports2.selectPageDown = selectPageDown;
    exports2.selectPageUp = selectPageUp;
    exports2.selectParentSyntax = selectParentSyntax;
    exports2.selectSubwordBackward = selectSubwordBackward;
    exports2.selectSubwordForward = selectSubwordForward;
    exports2.selectSyntaxLeft = selectSyntaxLeft;
    exports2.selectSyntaxRight = selectSyntaxRight;
    exports2.simplifySelection = simplifySelection;
    exports2.splitLine = splitLine;
    exports2.standardKeymap = standardKeymap;
    exports2.temporarilySetTabFocusMode = temporarilySetTabFocusMode;
    exports2.toggleBlockComment = toggleBlockComment;
    exports2.toggleBlockCommentByLine = toggleBlockCommentByLine;
    exports2.toggleComment = toggleComment;
    exports2.toggleLineComment = toggleLineComment;
    exports2.toggleTabFocusMode = toggleTabFocusMode;
    exports2.transposeChars = transposeChars;
    exports2.undo = undo;
    exports2.undoDepth = undoDepth;
    exports2.undoSelection = undoSelection;
  }
});
...
var require_dist9 = __commonJS({
  "node_modules/@lezer/lr/dist/index.cjs"(exports2) {
    ...
    exports2.ExternalTokenizer = ExternalTokenizer;
...
var require_dist10 = __commonJS({
  "node_modules/@lezer/css/dist/index.cjs"(exports2) {
    "use strict";
    Object.defineProperty(exports2, "__esModule", { value: true });
    var lr = require_dist9();
    var highlight = require_dist6();
    ...
    var identifiers = new lr.ExternalTokenizer(
      identifierTokens(identifier, VariableName, callee),
      { contextual: true }
    );
    var queryIdentifiers = new lr.ExternalTokenizer(
...
var require_dist11 = __commonJS({
  "node_modules/@codemirror/lang-css/dist/index.cjs"(exports2) {
    "use strict";
    var css$1 = require_dist10();
    var language = require_dist7();
    var common = require_dist5();
    ...
    var VariablesByNode = new common.NodeWeakMap();
    ...
    function variableNames(doc, node, isVariable) {
      if (node.to - node.from > 4096) {
        ...
        if (known)
          return known;
        let result = [], seen = /* @__PURE__ */ new Set(), cursor = node.cursor(common.IterMode.IncludeAnonymous);
    ...
    exports2.css = css2;
    exports2.cssCompletionSource = cssCompletionSource;
    exports2.cssLanguage = cssLanguage;
    exports2.defineCSSCompletionSource = defineCSSCompletionSource;
  }
});

// node_modules/@codemirror/theme-one-dark/dist/index.cjs
var require_dist12 = __commonJS({
  "node_modules/@codemirror/theme-one-dark/dist/index.cjs"(exports2) {
    "use strict";
    var view = require_dist4();
    var language = require_dist7();
    var highlight = require_dist6();
    ...
    var oneDarkTheme = view.EditorView.theme({
      "&": {
        color: ivory,
    ...
    var oneDarkHighlightStyle = language.HighlightStyle.define([
      {
        tag: highlight.tags.keyword,
        color: violet
      },
    ...
    var oneDark2 = [oneDarkTheme, language.syntaxHighlighting(oneDarkHighlightStyle)];
    exports2.color = color;
    exports2.oneDark = oneDark2;
    exports2.oneDarkHighlightStyle = oneDarkHighlightStyle;
    exports2.oneDarkTheme = oneDarkTheme;
  }
});

// ui/settings-panel/css-editor.js
var { EditorView, keymap, lineNumbers, highlightActiveLine } = require_dist4();
var { EditorState } = require_dist2();
var { defaultKeymap, history, historyKeymap } = require_dist8();
var { css } = require_dist11();
var { oneDark } = require_dist12();
