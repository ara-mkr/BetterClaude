...
class ThemeEngine {
  ...
  applySettings(settings) {
    ...
    const themeCSS = this.presets[settings.appearance.activeTheme] || "";
  ...
  setTheme(themeId) {
    ...
    const themeCSS = this.presets[themeId] || "";
    ensureStyleTag(THEME_STYLE_ID).textContent = themeCSS;
    syncClaudeColorModeFromThemeCSS(themeCSS);
  ...
  setAccentColor(color) {
    ...
    const bg = getComputedStyle(document.documentElement).getPropertyValue("--bc-bg").trim() || "#14101f";
    const ring = focusRingColor(color, bg);
