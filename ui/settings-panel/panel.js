  ...
  _renderThemes() {
    ...
    const shuffleBtn = el("button", {
      ...
      onclick: () => {
        ...
        this._set("appearance.activeTheme", pick);
        this.host.applyThemePreview(pick);
        this.renderSection();
      },
    });
    ...
    const importUrlBtn = el("button", {
      ...
      onclick: async () => {
        ...
        try {
          ...
          urlInput.value = "";
          this._set("appearance.activeTheme", result.id);
          this.host.applyThemePreview(result.id);
          this.renderSection();
        } catch (err) {
    ...
    this.host.listUserThemeIds().then((userIds) => {
      ...
      sortedIds.forEach((id) => {
        ...
        card.addEventListener("click", (e) => {
          ...
          this._set("appearance.activeTheme", id);
          this.host.applyThemePreview(id);
          this.renderSection();
        });
        card.appendChild(this._buildThemePreview(parseThemeVars(themesData[id])));

        const footer = el("div", { class: "bc-theme-card-footer" });
        footer.appendChild(el("span", { text: THEME_LABELS[id] || id }));
        ...
        const copyBtn = el("button", {
          ...
          onclick: async () => {
            const payload = JSON.stringify({ name: THEME_LABELS[id] || id, vars: parseThemeVars(themesData[id]) }, null, 2);
            try {
          ...
          actions.appendChild(el("button", {
            ...
            onclick: async () => {
              ...
              if (settings.appearance.activeTheme === id) {
                this._set("appearance.activeTheme", "midnight-violet");
                this.host.applyThemePreview("midnight-violet");
              }
              this.renderSection();
            },
          }));
