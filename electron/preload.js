...
const { insertIntoComposer, waitForComposer, buildTranscriptText, findComposer, findSendButton } = require("../core/compose-insert");
...
async function bootstrap() {
  ...
  // every inline custom property document.documentElement carries whose
  // name starts with "--bc-" — robust to that set changing upstream.
  function clearInlineAccentVariables() {
    ...
    for (let i = 0; i < style.length; i++) {
      const prop = style[i];
      if (prop.startsWith("--bc-")) bcProps.push(prop);
    }
  ...
  function setSetting(keyPath, value) {
    return ipcRenderer.invoke("settings:set", keyPath, value).then((updated) => {
      settings = updated;
      return updated;
    });
  }
  ...
  stopKonami = mountKonamiListener(() => {
    setSetting("personality.easterEggs.konamiUnlocked", true);
    setSetting("appearance.activeTheme", "secret-rainbow");
    themeEngine.setTheme("secret-rainbow");
  ...
  function runRadialAction(id) {
    if (id === "settings") settingsPanel.open();
      ...
      if (ids.length) {
        ...
        setSetting("appearance.activeTheme", pick);
        themeEngine.setTheme(pick);
      }
  ...
  document.addEventListener("click", () => soundEngine.play("click"));
  ...
  async function surpriseMe() {
    ...
    await applyBundle(bundle, { setSetting, applyBundlePreview });
    if (settings.motion.confetti) celebrate();
  }
  ...
  const panelHost = {
    ...
    applyMood: async (mood) => {
      ...
      if (bundle) await applyBundle(bundle, { setSetting, applyBundlePreview });
    },
  ...
  const logoSrc = "data:image/png;base64," + fs.readFileSync(path.join(__dirname, "../assets/icon.png")).toString("base64");
